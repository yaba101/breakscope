...
export async function GET(
  ...
  try {
    ...
    return Response.json({
      project: (await ownedProject(request, projectId)).project,
    });
  } catch (error) {
...
export async function PATCH(
  ...
  try {
    ...
    await env.DB.prepare(
      "UPDATE project SET name = ?, baseline_origin = ?, candidate_origin = ?, updated_at = ? WHERE id = ?",
    )
      .bind(
        input.name,
        input.baselineUrl,
...
export async function DELETE(
  ...
  try {
    ...
    const artifacts = await env.DB.prepare(
      "SELECT baseline_key, candidate_key, diff_key FROM run WHERE project_id = ?",
    )
      .bind(projectId)
      .all<{
    ...
    const objectKeys = artifacts.results.flatMap(
      (artifact: {
        baseline_key: string | null;
        candidate_key: string | null;
        diff_key: string | null;
    ...
    await env.DB.batch([
      env.DB.prepare(
        "DELETE FROM region WHERE run_id IN (SELECT id FROM run WHERE project_id = ?)",
      ).bind(projectId),
      env.DB.prepare(
        "DELETE FROM share WHERE run_id IN (SELECT id FROM run WHERE project_id = ?)",
      ).bind(projectId),
