...
function WorkspaceControls({ retainedBytes, history, disabled, onOpenRun, onExport, onClearEvidence, onClearPreferences, onResetWorkspace }: { retainedBytes: number; history: LocalScanRun[]; disabled: boolean; onOpenRun: (run: LocalScanRun) => void; onExport: (format: "md" | "json" | "html") => void; onClearEvidence: () => void; onClearPreferences: () => void; onResetWorkspace: () => void }) {
  ...
  return <main id="main-content" className="breakscope-shell bk-workspace-page">
    ...
    {(routes.length > 0 || issues.length > 0 || scanning) ? <section className={`bs-workspace ${configCollapsed ? "config-collapsed" : ""}`} style={{ "--bk-inspector-width": `${inspectorWidth}px` } as CSSProperties}>
      <aside className="bs-config" aria-label="Scan configuration">
        ...
        {!configCollapsed && <div className="bk-config-actions"><WorkspaceControls retainedBytes={retainedEvidenceBytes} history={workspaceQuery.data?.scanHistory ?? []} disabled={scanning} onOpenRun={(run) => void openHistoryRun(run)} onExport={(format) => { const state = queryClient.getQueryData<BreakscopeState>(breakscopeQueryKeys.workspace()); if (state) downloadLocalReport(format, state); }} onClearEvidence={() => void clearCapturedEvidence()} onClearPreferences={() => void clearViewPreferences()} onResetWorkspace={() => void resetWorkspace()} /></div>}
      </aside>
