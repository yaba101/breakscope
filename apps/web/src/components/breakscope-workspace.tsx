  ...
  return <main id="main-content" className="breakscope-shell bk-workspace-page">
    ...
    {(routes.length > 0 || issues.length > 0 || scanning) ? <section className={`bs-workspace ${configCollapsed ? "config-collapsed" : ""}`} style={{ "--bk-inspector-width": `${inspectorWidth}px` } as CSSProperties}>
      <aside className="bs-config" aria-label="Scan configuration">
        ...
        {!configCollapsed && <div className="bk-config-actions"><WorkspaceControls retainedBytes={retainedEvidenceBytes} history={workspaceQuery.data?.scanHistory ?? []} diagnosticsHistory={workspaceQuery.data?.diagnosticsHistory ?? []} currentIssues={issues} disabled={scanning} onExport={(format) => { const state = queryClient.getQueryData<BreakscopeState>(breakscopeQueryKeys.workspace()); if (state) downloadLocalReport(format, state); }} onDiagnosticSample={(sample) => void recordDiagnosticSample(sample)} onClearEvidence={() => void clearCapturedEvidence()} onClearPreferences={() => void clearViewPreferences()} onResetWorkspace={() => void resetWorkspace()} /></div>}
      </aside>
