...
function ViewportIssueInspector({ width, routePath, issues, pageWideIssues, baselineIssues, baselineAvailable, selectedIssue, aiAnalysis, aiPending, aiError, retesting, url, onSelect, onClearSelection, onRetest, onAnalyze }: { width: number; routePath: string; issues: ResponsiveIssue[]; pageWideIssues: ResponsiveIssue[]; baselineIssues: ResponsiveIssue[]; baselineAvailable: boolean; selectedIssue?: ResponsiveIssue; aiAnalysis?: AiIssueAnalysis; aiPending: boolean; aiError?: string; retesting: boolean; url: string; onSelect: (issue: ResponsiveIssue) => void; onClearSelection: () => void; onRetest: () => void; onAnalyze: (issue: ResponsiveIssue, mode: "concise" | "technical") => void }) {
  ...
  const changeCounts = useMemo(() => ({
    new: allFindings.filter((issue) => findingChange(issue, baselineIssues) === "new").length,
    regressed: allFindings.filter((issue) => findingChange(issue, baselineIssues) === "regressed").length,
    existing: allFindings.filter((issue) => findingChange(issue, baselineIssues) === "existing").length,
  ...
  return <div className="bk-viewport-inspector">
    ...
    {baselineAvailable ? <nav className="bk-change-views" aria-label="Baseline status"><button type="button" className={changeView === "changes" ? "active" : ""} aria-pressed={changeView === "changes"} onClick={() => { onClearSelection(); setChangeView("changes"); }}><span>Changes</span><small>{changeCounts.new} new · {changeCounts.regressed} regressed</small></button><button type="button" className={changeView === "fixed" ? "active" : ""} aria-pressed={changeView === "fixed"} onClick={() => { onClearSelection(); setChangeView("fixed"); }}><span>Fixed</span><small>{changeCounts.fixed}</small></button><button type="button" className={changeView === "existing" ? "active" : ""} aria-pressed={changeView === "existing"} onClick={() => { onClearSelection(); setChangeView("existing"); }}><span>Existing</span><small>{changeCounts.existing}</small></button><button type="button" className={changeView === "all" ? "active" : ""} aria-pressed={changeView === "all"} onClick={() => { onClearSelection(); setChangeView("all"); }}><span>All</span><small>{allFindings.length}</small></button></nav> : allFindings.length > 0 && <p className="bk-baseline-empty"><Flag size={14} /> Set a trusted scan as the baseline in Changes to separate new regressions from existing findings.</p>}
