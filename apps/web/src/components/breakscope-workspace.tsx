...
import { clearBreakscopeState, loadBreakscopeState, saveBreakscopeState, type BreakscopeState, type LocalScanRun, type PersistedScanJob, type PersistedViewportPreview } from "@/lib/breakscope-workspace";
...
function WorkspaceControls({ retainedBytes, history, currentIssues, disabled, onOpenRun, onExport, onClearEvidence, onClearPreferences, onResetWorkspace }: { retainedBytes: number; history: LocalScanRun[]; currentIssues: ResponsiveIssue[]; disabled: boolean; onOpenRun: (run: LocalScanRun) => void; onExport: (format: "md" | "json" | "html") => void; onClearEvidence: () => void; onClearPreferences: () => void; onResetWorkspace: () => void }) {
  const [moreOpen, setMoreOpen] = useState(false);
  const [confirmingReset, setConfirmingReset] = useState(false);
  const comparisonRun = history.find((run) => run.issues !== currentIssues && run.issues.some((issue) => !currentIssues.some((current) => current.fingerprint === issue.fingerprint))) ?? history[1];
  const currentFingerprints = new Set(currentIssues.map((issue) => issue.fingerprint));
  const previousFingerprints = new Set(comparisonRun?.issues.map((issue) => issue.fingerprint) ?? []);
  const comparison = comparisonRun ? { newCount: currentIssues.filter((issue) => !previousFingerprints.has(issue.fingerprint)).length, fixedCount: comparisonRun.issues.filter((issue) => !currentFingerprints.has(issue.fingerprint)).length, unchangedCount: currentIssues.filter((issue) => previousFingerprints.has(issue.fingerprint)).length } : undefined;
  return <Popover.Root>
    ...
    <Popover.Portal><Popover.Content className="bk-workspace-controls" side="right" align="end" sideOffset={10} aria-label="Workspace controls">
      ...
      {moreOpen && <div className="bk-workspace-control-more"><section className="bk-local-storage-summary"><b>Saved on this device</b><span>Test setup, presets, screenshots, issue evidence, and view preferences.</span><small>AI repair briefs remain in memory for this session only.</small></section>{comparison && <section className="bk-run-comparison"><header><b>Changes since previous run</b><small>{new Date(comparisonRun!.createdAt).toLocaleString()}</small></header><div><span className="new"><b>{comparison.newCount}</b>New</span><span className="fixed"><b>{comparison.fixedCount}</b>Fixed</span><span><b>{comparison.unchangedCount}</b>Unchanged</span></div></section>}{history.length > 0 && <section className="bk-run-history"><b><Clock3 size={14} /> Recent runs</b>{history.map((run) => <button type="button" key={run.id} onClick={() => onOpenRun(run)}><span>{new Date(run.createdAt).toLocaleString()}</span><small>{run.issues.length} issues · {run.target.selectedRoutes.length} routes</small></button>)}</section>}<section className="bk-report-export"><b><Download size={14} /> Export local report</b><div><button type="button" onClick={() => onExport("md")}>Markdown</button><button type="button" onClick={() => onExport("json")}>JSON</button><button type="button" onClick={() => onExport("html")}>HTML + images</button></div></section><button type="button" disabled={disabled} onClick={onClearPreferences}><Settings2 size={15} /> Reset view preferences</button><button type="button" onClick={() => window.location.reload()}><RefreshCw size={15} /> Reload workspace</button>{process.env.NODE_ENV === "development" && <DevRuntimePanel retainedBytes={retainedBytes} />}</div>}
...
function PreviewSurface({ scanning, previewRef, scrollProgress, issuePosition, onScroll, onUserInteraction, children }: { scanning?: boolean; previewRef?: RefObject<HTMLDivElement | null>; scrollProgress: number; issuePosition?: number; onScroll?: () => void; onUserInteraction?: () => void; children: ReactNode }) {
  ...
  return <div className="bk-library-screen-content"><div className="bs-preview" ref={previewRef} onScroll={onScroll} onWheel={onUserInteraction} onPointerDown={onUserInteraction}>{children}</div>{scanning && <div className="bk-scan-overlay"><i /><span>Scanning section {section} of 8</span></div>}{(scanning || issuePosition !== undefined) && <div className={`bk-page-minimap ${scanning ? "is-scanning" : "has-issue"}`} aria-label={`${Math.round(scrollProgress * 100)}% through page`}><span className="bk-minimap-progress" style={{ height: `${Math.max(8, scrollProgress * 100)}%` }} />{Array.from({ length: 8 }, (_, index) => <span key={index} className={`bk-minimap-section ${index < section ? "scanned" : ""}`} style={{ top: `${index * 12.5}%` }} />)}{issuePosition !== undefined && <b className="bk-minimap-issue" style={{ top: `${Math.min(96, Math.max(1, issuePosition * 100))}%` }} />}</div>}</div>;
}
...
export function BreakscopeWorkspace() {
  ...
  async function openHistoryRun(run: LocalScanRun) {
    const stored = queryClient.getQueryData<BreakscopeState>(breakscopeQueryKeys.workspace()) ?? await loadBreakscopeState();
    const next = { ...stored, target: run.target, latestIssues: run.issues, latestPreviews: run.previews, updatedAt: Date.now() };
  ...
  return <main id="main-content" className="breakscope-shell bk-workspace-page">
    ...
    {(routes.length > 0 || issues.length > 0 || scanning) ? <section className={`bs-workspace ${configCollapsed ? "config-collapsed" : ""}`} style={{ "--bk-inspector-width": `${inspectorWidth}px` } as CSSProperties}>
      <aside className="bs-config" aria-label="Scan configuration">
        ...
        {!configCollapsed && <div className="bk-config-actions"><WorkspaceControls retainedBytes={retainedEvidenceBytes} history={workspaceQuery.data?.scanHistory ?? []} currentIssues={issues} disabled={scanning} onOpenRun={(run) => void openHistoryRun(run)} onExport={(format) => { const state = queryClient.getQueryData<BreakscopeState>(breakscopeQueryKeys.workspace()); if (state) downloadLocalReport(format, state); }} onClearEvidence={() => void clearCapturedEvidence()} onClearPreferences={() => void clearViewPreferences()} onResetWorkspace={() => void resetWorkspace()} /></div>}
      </aside>
