...
export function ReportViewer({ token }: { token: string }) {
  ...
  return <main id="main-content" className="breakscope-shell bk-workspace-page">
    <header className="bk-command-bar"><div className="bk-command-brand"><BreakscopeLogo variant="dark" /><span>Report</span></div><div className="bk-command-target"><span>{target ? new URL(target.url).host : "Unknown"}</span><code>{target?.url ?? ""}</code></div><div className="bk-command-actions"><span className={`bk-command-state complete`}>{issueFamilyCount ? `${issueFamilyCount} issue ${issueFamilyCount === 1 ? "family" : "families"}` : "All checks passed"}</span></div></header>
