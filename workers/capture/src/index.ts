...
async function capture(
  url: string,
  viewport: CaptureRunV1["viewport"],
  env: Env,
) {
...
export default {
  async queue(batch: MessageBatch<CaptureRunV1>, env: Env) {
    for (const message of batch.messages) {
      ...
      try {
        ...
        await Promise.all([
          env.IMAGES.put(`${baseKey}/baseline.png`, baseline, {
            httpMetadata: { contentType: "image/png" },
          }),
          env.IMAGES.put(`${baseKey}/candidate.png`, candidate, {
            httpMetadata: { contentType: "image/png" },
          }),
        ]);
        await env.DB.prepare(
          ...
          .bind(
            `${baseKey}/baseline.png`,
            `${baseKey}/candidate.png`,
            Date.now(),
            job.runId,
          )
          .run();
  ...
  async scheduled(_controller: ScheduledController, env: Env) {
    ...
    const expired = await env.DB.prepare(
      "SELECT id, baseline_key, candidate_key, diff_key FROM run WHERE expires_at < ? LIMIT 100",
    )
      .bind(now)
      .all<{
        id: string;
    ...
    const runIds = expired.results.map((run) => run.id);
    const objectKeys = expired.results.flatMap((run) =>
      [run.baseline_key, run.candidate_key, run.diff_key].filter(
    ...
    if (runIds.length) {
      ...
      await env.DB.batch([
        env.DB.prepare(
          `DELETE FROM share WHERE run_id IN (${placeholders})`,
        ).bind(...runIds),
        env.DB.prepare(
          `DELETE FROM region WHERE run_id IN (${placeholders})`,
        ).bind(...runIds),
        env.DB.prepare(`DELETE FROM run WHERE id IN (${placeholders})`).bind(
          ...runIds,
        ),
      ]);
    }
    await env.DB.prepare(
      "DELETE FROM share WHERE expires_at < ? OR revoked_at IS NOT NULL",
    )
      .bind(now)
      .run();
  },
